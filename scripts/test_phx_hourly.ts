...
async function runPHXHourlyTest() {
  console.log(`\n>>> STARTING PHX HOURLY TEST (Target: 35 items) <<<\n`);
  console.log(`Criteria: >$12k, <1h old, Private Seller, Phoenix (200mi)\n`);
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        console.log(`\n>>> FINISHED PHX HOURLY TEST <<<\n`);
        resolve();
      } else {
        console.error(`\n>>> FAILED PHX HOURLY TEST (exit code ${code}) <<<\n`);
...
runPHXHourlyTest().catch(console.error);
