...
// Rotating SoCal locations to get fresh feeds
const LOCATIONS = [
...
const RADIUS = '10'; // Smaller radius for fresher, localized feeds
...
async function runBatch() {
  ...
  console.log(`Starting batch run with location rotation. Target: ${TARGET_INSERTIONS} listings.`);
  ...
  for (const batch of BATCHES) {
    ...
    // Rotate location for each batch
    const loc = LOCATIONS[locIndex % LOCATIONS.length];
    locIndex++;

    console.log(`\nRunning for ${batch.make} [${batch.models}] in ${loc.name}...`);
    
    const env = {
      ...process.env,
      OU_LAT: loc.lat,
      OU_LNG: loc.lng,
      OU_RADIUS_MILES: RADIUS,
